 ...
 */
# define ZPC_HMAC_KEY_TYPE_CCA_DATA        1
# define ZPC_HMAC_KEY_TYPE_CCA_CIPHER      2
# define ZPC_HMAC_KEY_TYPE_EP11            3
 ...
 * \param[in] func HMAC hash function: sha-224, sha-256, sha-384, or sha-512
...
/**
 * Import an HMAC secure key or retrievable secret ID.
 * \param[in,out] key HMAC key
 * \param[in] seckey HMAC secret ID
 * \param[in] seckeylen HMAC key secret ID length [bytes]
 * \return 0 on success. Otherwise, a non-zero error code is returned.
 */
__attribute__((visibility("default")))
int zpc_hmac_key_import(struct zpc_hmac_key *key, const unsigned char *seckey,
    size_t seckeylen);
/**
...
/**
 * Export an HMAC secure key or retrievable secret ID.
 * \param[in,out] key HMAC key
 * \param[out] seckey HMAC retrievable secret ID
 * \param[in,out] seckeylen secret ID length [bytes]
 * \return 0 on success. Otherwise, a non-zero error code is returned.
 */
__attribute__((visibility("default")))
int zpc_hmac_key_export(struct zpc_hmac_key *key, unsigned char *seckey,
    size_t *seckeylen);
/**
